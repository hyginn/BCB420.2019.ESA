...
#' bonferroni.
#' 
#' @param data dataframe contain the mapping of gene and reactome data
#' 
...
#' result <- PathwayEnrichment(sampleGeneList,"BH",tmp)
#'
#' @export
PathwayEnrichment<-function(geneSet,method,data){
  #fetch HGNC reference genes 
  pathway_df<-data.frame(pathway=c(),pval=c())
  unique_pw<-unique(data$REACTOME_ID)
  ...
  for(gene in geneSet){
    pws<-data$REACTOME_ID[data$HGNC==gene]
    for(pw in pws){
      if(!is.na(pw)){
        pwList<-c(pwList,pw)
      }
  ...
  for( pathway in pwList){
    ...
    occur<-sum(data$REACTOME_ID==pathway)
    occurence[[pathway]]<-occur
    geneList<-data$HGNC[data$REACTOME_ID==pathway]
    ...
    fisher_matrix[2,2]<-length(data$REACTOME_ID) -occur
