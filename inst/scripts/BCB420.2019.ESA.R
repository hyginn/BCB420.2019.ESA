...
# ====  PARAMETERS  ============================================================
# hgnc: The HGNC symbol that the user would like to analyze by comparing
# the pairwise sequence alignments with it's interacting genes/proteins
...
getAlignments <- function(hgnc) {
  # Purpose:
  #     Return a data frame containing the alignment data between the provided
  #     hgnc gene and it's corresponding genes based on the STRING
  #     database.
  #
  # Parameters:
  #     hgnc: The hgnc symbol of the gene of interest in the form of a
  ...
  #
  # Value:
  #     result: A data frame including the hgnc gene of interest,
...
writeData <- function(result) {
  ...
  # Purpose:
  #     Write the resulting data frame data from the getAlignments()
  #     function in the script as a .tsv file
  #
  # Parameters:
  #     result: The resulting data frame containing the alignment data
  #     from getAlignments().
  #
  # Value:
  #     A .tsv file containing the sample data
  #
  # code

  writeLines(c("TranscriptA\t TranscriptB\t
               SequenceA\t SequenceB\t ConsensusSequence\t
               Scorematrix",
               sprintf("%s\t%s\t%s\t%s\t%s\t%s\n",
                       result$a, result$b, result$SequenceA,
                       result$SequenceB, result$Consensus, result$Score)),
             con = "inst/extdata/sampleSetESA.tsv")
...
# ====  SAMPLE  ===============================================================
# This is a sample script that shows how the ESA tool works

# Picking a sample hgnc symbol to get the corresponding protein alignments
# with it's interacting genes
hgnc <- "BECN1"

## Run all the code under the 'PACKAGES' section of the script ##
